<nav class="navbar navbar-expand-lg navbar-custom">

<div class="container custom-container navbar-container">
    
        <a
            class="navbar-brand d-flex align-items-center gap-2"
            href="#">

            <img
                src="{{ asset('images/SedesCompleto.png') }}"
                class="navbar-logo-sedes">

            <img
                src="{{ asset('images/LogoJornada.png') }}"
                class="navbar-logo">

            <p class="navbar-text">
                Jornada de Innovación <br>
                Practica Docente
            </p>
        </a>

        @if(empty($inscripcion))

        <button
            class="navbar-toggler"
            type="button"
            data-bs-toggle="collapse"
            data-bs-target="#navbarContent">

            <span class="navbar-toggler-icon"></span>

        </button>

        <div
            class="collapse navbar-collapse justify-content-end"
            id="navbarContent">

            <ul class="navbar-nav align-items-lg-center gap-lg-3">

                    <li class="nav-item">
                        <a class="nav-link nav-link-custom" href="#sobre">
                            Información
                        </a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link nav-link-custom" href="#anteriores">
                            Jornadas
                        </a>
                    </li>

                    <li class="nav-item">
                        <a class="nav-link nav-link-custom" href="#cronograma">
                            Cronograma
                        </a>
                    </li>

                    <li class="nav-item mt-3 mt-lg-0">

                        <a
                            href="{{ route('inscription') }}"
                            class="btn btn-inscribirse">

                            Inscribirse

                        </a>

                    </li>

            </ul>

        </div>

        @else

            <a
                class= "justify-content: center">

                <a
                    href="{{ route('home') }}"
                    class="btn btn-inscribirse">

                        Pagina Principal

                    </a>

            </a>

        @endif

</div>

</nav>